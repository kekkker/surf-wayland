...
act_zoom(const Arg *a)
{
	...
	if (a->i > 0)
		webkit_web_view_set_zoom_level(t->wv, cur + 0.1);
	else if (a->i < 0)
		webkit_web_view_set_zoom_level(t->wv, cur - 0.1);
	else
		webkit_web_view_set_zoom_level(t->wv, 1.0);
}

/* ── find ────────────────────────────────────────────────────────────────── */

void
act_find_next(const Arg *a)
{
	Tab *t = app_active_tab();
	if (!t || !t->finder)
		return;
...
static void
js_eval_simple(WebKitWebView *wv, const char *js)
{
...
void
find_select_yank_cb(GObject *obj, GAsyncResult *res, gpointer ud)
{
	(void)res;
	(void)ud;
	GError *err = NULL;
...
void
act_find_select_enter(const Arg *a)
{
	(void)a;
	Tab *t = app_active_tab();
	if (!t || t->find_match_count <= 0)
		return;
	t->mode = MODE_SELECT;
	char js[256];
	snprintf(js, sizeof js,
...
void
act_find_select_line(const Arg *a)
{
	(void)a;
	Tab *t = app_active_tab();
	if (!t || t->find_match_count <= 0)
...
void
act_find_select_exit(const Arg *a)
{
	(void)a;
	Tab *t = app_active_tab();
	if (!t)
...
act_new_tab(const Arg *a)
{
	...
	tabarray_new(&g_app.tabs, WPE_DISPLAY(g_app.sdisplay), g_app.toplevel,
				 NULL /* related_view */, tab_changed_cb, g_app.tab_close_fn, NULL);
	app_repaint_chrome();
}

void
act_close_tab(const Arg *a)
{
	(void)a;
...
act_tab_reopen(const Arg *a)
{
	...
	Tab *t = tabarray_new(&g_app.tabs, WPE_DISPLAY(g_app.sdisplay),
						  g_app.toplevel, NULL /* related_view */,
...
clipboard_set(const char *text)
{
	...
	GSubprocess *p = g_subprocess_new(G_SUBPROCESS_FLAGS_STDIN_PIPE,
									  &err, "wl-copy", NULL);
	if (err) {
		g_warning("wl-copy: %s", err->message);
		g_error_free(err);
		return;
	}
...
settings_apply(struct Tab *t)
{
	...
	/* Statusbar overlaps bottom of the view — push web content up. */
	{
		...
		snprintf(pad_css, sizeof(pad_css),
				 "html{padding-bottom:%dpx !important;}\n",
				 CHROME_STATUSBAR_H);
...
void
act_toggle_setting(const Arg *a)
{
	int id = a->i;
	if (id < 0 || id >= SET_LAST)
		return;
...
void
act_toggle_cookies(const Arg *a)
{
	(void)a;
...
static void
wl_paste_cb(GObject *src, GAsyncResult *res, gpointer ud)
{
	(void)ud;
	GError *err = NULL;
	GBytes *out = NULL;
	...
	if (*buf) {
		Tab *t = app_active_tab();
		if (t)
			webkit_web_view_load_uri(t->wv, buf);
	}
	g_free(buf);
	g_bytes_unref(out);
}
...
act_clipboard(const Arg *a)
{
	...
	if (a->i == 0) {
		...
		GSubprocess *p = g_subprocess_new(G_SUBPROCESS_FLAGS_STDOUT_PIPE,
										  &err, "wl-paste", "-n", NULL);
		if (err) {
			g_warning("wl-paste: %s", err->message);
			g_error_free(err);
			return;
		}
...
void
act_show_instance_id(const Arg *a)
{
	(void)a;
	Tab *t = app_active_tab();
