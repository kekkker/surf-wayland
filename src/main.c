...
{
    /* Re-stack all chrome panels above the active tab's wl_surface.
     * Other tab subsurfaces sit underneath the active one and stay
     * hidden via z-order. */
...
void app_layout(int W, int H)
{
    ...
    /* WPE view — fills the middle. Each tab has its own subsurface; set
     * position on every tab so their content lands in the same rect.
     * (Inactive tabs are below in z-order, so their position only
     * matters when the user switches to them.) */
...
int main(int argc, char *argv[])
{
     ...
     *    surf_view_realize() (called from tabarray_new). No shared
    ...
    /* Tab subsurfaces are owned by each SurfView and freed via
     * tabarray_close → g_object_unref → surf_view_dispose. */
